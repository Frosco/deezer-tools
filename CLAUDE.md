...
- **Baseline pacer between every delete.** `pacedSleep` waits `defaultPace ± defaultPaceJitter` (`1s ± 200ms`) before *every* delete attempt, including the first. This exists so even on a happy path we don't burst hard enough to look like a bot. The values are package-level vars rather than `Options` fields — production is intentionally untunable; the test binary zeroes them in `init()`. Don't remove the pacer to "make it faster" — it's the throttle that keeps us off Akamai's bot list.
